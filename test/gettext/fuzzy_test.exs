defmodule Gettext.FuzzyTest do
  ...
  test "jaro_distance/2" do
    assert Fuzzy.jaro_distance({:regular, "foo"}, {:plural, "foo", "bar"}) == 1.0
    assert Fuzzy.jaro_distance({:plural, "foo", "bar"}, {:regular, "foo"}) == 1.0
    assert Fuzzy.jaro_distance({:regular, "foo"}, {:regular, "foos"}) > 0.0
    assert Fuzzy.jaro_distance({:plural, "foo", ""}, {:plural, "bar", ""}) == 0.0
  end
